...
function ProductList({ onHomeClick }) {
  ...
  const isInCart = (plant, category) => {
    const itemId = `${category}-${plant.name}`;
  ...
  const handleAddToCart = (plant, category) => {
    // Create a unique ID and add the plant to cart
    const plantWithDetails = {
      ...plant,
      id: `${category}-${plant.name}`,
      category,
      quantity: 1,
    };

    dispatch(addToCart(plantWithDetails));
  };
      ...
      <header>
        ...
        <div className="nav-buttons">
          ...
          <button onClick={() => setShowCart(!showCart)}>
            {showCart ? "View Plants" : `View Cart (${cartItemCount})`}
          </button>
        </div>
      </header>
              ...
              <div className="plants-grid">
                {categoryObj.plants.map((plant, i) => (
                  <div key={i} className="plant-card">
                    ...
                    <button
                      onClick={() =>
                        handleAddToCart(plant, categoryObj.category)
                      }
                      disabled={isInCart(plant, categoryObj.category)}
                      className={
                        isInCart(plant, categoryObj.category)
                          ? "button-disabled"
                          : ""
                      }
                    >
